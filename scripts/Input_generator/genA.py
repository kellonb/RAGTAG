...
# Module written by kbelfon, useful functions for RAGTAG, uses pytraj
#++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++#
...
def circular_avg(angle_array):
    ...
    a = b = 0
    conv = 3.14159265359 / 180.00000000  # pi/180
    i_conv = 180.00000000 / 3.14159265359  # 180/pi
...
def make_chirst(atom_num_array, r1_array, r23_array, r4_array, rk23_array, filename, flag):
    ''' This assumes that r2 and r3 equals and rk2 and rk3 equals
        flag == S means only a single dihedral angle is restrain, 
        flag == M means multiple dihedral restrain angles '''
    fin = open(filename, "w")
...
# load MM structures or QM structures to calculate dihedral (structures
# must be loaded as a trajectory)
...
def get_all_dihedral(top, flag):
    ...
    # get atoms index for all dihedral from parmed -n ensure the logo is not
    # printed
    ...
    # os.system('sed -i  "s/I/ /" dih_out.log') # remove I in the file,
    # uncomment if you want the index for improper dihedrals
    ...
    dihinfo = np.genfromtxt('dih_out.log', dtype=str,
    ...
    if Flag == 0:
        ...
        with open(filename) as file_handle:
            for line in file_handle:
                # May need to change this line if you you are not using MP2
                # energy
                if line.rstrip() == "The Calculated Surface using the MP2 energy":
                    # extract nconf of lines after this
                    qme = (list(islice(file_handle, nconf)))
                    for n in qme:
                        # append lines with both energy and number
                        data.append(n)
                    break
...
def extract_MMenergy(mol_name, nconf, Flag, filename=None, file_output='info'):
    ...
    #---------------------------------------------------------------------------------------#
    # structure name was created as mol_name.000 to
    # mol_name.nconf(mol_name.012)
    if Flag == 0:
                ...
                with open("%s.out" % (struct)) as file_handle:
                    for line in file_handle:
                        ...
                        if "      A V E R A G E S   O V E R" in line:
                            # extract 8 lines after this
                            mmdata = (list(islice(file_handle, 8)))
                            for n in mmdata:
                                # append lines with both energy and number
                                data.append(n)
                            break
...
def print_example():
    example_genA_input = '''
-dih1_name dih1_atom_types fitting_group_index periodicity
-dih2_name dih2_atom_types fitting_group_index periodicity
-dihN_name dihN_atom_types fitting_group_index periodicity
dataset_name <weight dih1_name dih2_name dihN_name >number_of_dihedrals_in_dataset
...
-chi2 CX-2C-2C-SE -0 4 2 
-chi3 2C-2C-SE-CT -0 4 3 2 1
MSEalpha <1 chi1 chi2 chi3 >3
...
def make_genA_input(num_dihedral, mol_name, dihname, dihatom, dihedrals, QMeng, MMeng, nbreaks, nFg, periodicity, weight=1):
    '''
    ...
    nFg is the fitting group the dihedral belongs to 
    periodicity is the periodcity of the cosines to fit the dihedral
    weight is applied to the AAE 
    '''
    # for cases when you have directory as part of mol name, normally when
    # dealing with 1 dataset
    if nbreaks == 1:
        a = mol_name.split("/")
        mol_name = a[-1]
    ...
    with open("genA_input", "w") as genA:
        ...
        if num_dihedral == 1 and nbreaks == 1:
            genA.write("%s" % (string_format_atype(dihname, dihatom)))
            genA.write(" -%s %s\n" % (nFg, periodicity))
            genA.write("%s" % (string_format_molname(mol_name)))
            genA.write(" <%s" % (weight))
            genA.write("%s" % (string_format_dihname(dihname)))
            genA.write(" >%s\n" % (num_dihedral))
        ...
        if num_dihedral == 1 and nbreaks > 1:
            indxbrk = np.arange(0, nbreaks, 1)  # indedx into weights
            for dih, atom in zip(dihname[0:num_dihedral], dihatom):
                # write the dihname and atom type
                genA.write("%s" % (string_format_atype(dih, atom)))
                # only one dihedral so for all dataset this is the same
                genA.write(" -%s %s\n" % (nFg[0], periodicity[0]))
            # get length of molname strings for the input to make the format
            # perfect
            genA.write("%s" % (string_format_molname(mol_name[0])))
            if (len(weight) == 1):
                genA.write(" <%s" % (weight))
            else:
                # first element in weight, 1st weight of dataset
                genA.write(" <%s" % (weight[0]))
            ...
            genA.write(" >%s\n" % (num_dihedral))
            data = {}
            # dihedrals is an arrray of arrays holding dihedrals
            for j in np.arange(0, (len(dihedrals)), 1):
                data[j] = dihedrals[j]
            ...
            # dihedral values are separated based on datasets so using for i in
            # nbreaks
            ...
            for nb in np.arange(0, nbreaks, 1):
                for d1 in np.arange(0, (len(all_values[nb])), 1):
                    genA.write('%10.4f ' %
                               (all_values[nb][r1][d1]))  # dihedrals
                    genA.write('%16.4f ' % (all_values[nbreaks][nb][d1]))  # QM
                    genA.write('%10.4f' %
                               (all_values[nbreaks + 1][nb][d1]))  # MM
                genA.write("\n")
                genA.write("/\n")
                if nb != (nbreaks - 1):  # dont write it for the last nbreaks
                    genA.write("%s" %
                               (string_format_molname(mol_name[nb + 1])))
                    if (len(weight) == 1):
                        genA.write(" <%s" % (weight))
                    else:
                        genA.write(" <%s" % (weight[nb]))
        ...
        if num_dihedral > 1 and nbreaks == 1:
            # indedx into the FG and periodicities per dihedral
            indxdih = np.arange(0, num_dihedral, 1)
            for dih, atom, indxd in zip(dihname, dihatom, indxdih):
                # write the dihname and atom type
                genA.write("%s" % (string_format_atype(dih, atom)))
                genA.write(" -%s %s\n" % (nFg[indxd], periodicity[indxd]))
            # get length of molname strings for the input to make the format
            # perfect
            genA.write("%s" % (string_format_molname(mol_name)))
            if (len(weight) == 1):
        ...
        if num_dihedral > 1 and nbreaks > 1:
            ...
            # get the number of columns and number of rows
            # number of columns is dih for nbreaks, then QM then MM
            num_cols = (len(all_values))
            # number of dihderals in a dataset (nbreak)
            num_rows1 = (len(all_values[0]))
            # number of QM and MM values which is based on the number of
            # dataset (nbreak)
            num_rows2 = (len(all_values[nbreaks]))
            ...
            # the last two cols (QM and MM) will have 2 rows(num_rows2=2),
            # where each row have the QM and MM valuses for each dataset
...
def run_MMenergy(top, maxcyc, chirst_name, infilename, outfilename, min_filename):
    ...
    # outfilename is the name for your amber outputfile eg. filename.rst7,
    # filename.out
...
'''
    ...
    with open("%s.in" % (min_filename), "w") as f:
        ...
        f.close()
    os.system("$AMBERHOME/bin/sander -O -i ./%s.in -p ./%s -c ./%s.rst7 -ref ./%s.rst7 -o ./%s.out -r ./%s.rst7 -inf ./%s.info" %
              (min_filename, top, infilename, infilename, outfilename, outfilename, outfilename))

# specific for proteins relaxing the back bone to an average chi.rst
# (chirst_name)
...
def suspect_list(vdw_file, elec_file, output_filename):
    ...
    # If there is 1 suspect structure in elec but multiple supect structures
    # in vdw
    ...
    if (elecin.size > 1 and vdwin.size > 1):
        for struct1 in vdwin:
            ...
            for struct2 in elecin:
                ...
                # it is a suspect structure is because it failed both vdw and
                # elec criteria
                if struct1 == struct2:
                    output.write("%18s %6s %6s\n" % (struct1, y, y))
                    suspect_array.append(struct1)
    ...
    with open(output_filename) as f1:
        for line in f1:
            ...
            if not line.startswith("#"):
                ...
                list_a.append(dict_key)
                list_b.append(dict_elem)
...
'''
        ...
        with open("1min.in", "w") as f:
            f.write(min_template.format(**info))
            f.close()
        os.system("$AMBERHOME/bin/sander -O -i ./1min.in -p ./%s -c ./%s -ref ./%s -o ./1min.out -r 1min.r" %
                  (top, inpcrd, inpcrd))
    else:
        min_template = '''minimizing initial structure
&cntrl
...
'''
        ...
        with open("2mdheat.in", "w") as f:
            ...
            f.close()
        os.system(
            "$AMBERHOME/bin/sander -O -i ./2mdheat.in -p ./%s -c 1min.r -ref ./1min.r -o ./2md.out -r 2md.r" % (top))
    else:
...
'''
        ...
        with open("prod.in", "w") as f:
            ...
            f.close()
        os.system(
            "$AMBERHOME/bin/sander -O -i ./prod.in -p ./%s -c 2md.r -o ./prod.out -x prod.nc -r prod.r" % (top))
    else:
