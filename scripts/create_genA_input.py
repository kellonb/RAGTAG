...
##-----------------Get user inputs-------------------------##
  ...
  arguments = ['nbreaks', 'mol_name', 'nconf', 'dihnum', 'flag_mm_read', "mm_read_file", 'flag_qm_read', "qm_read_file", 'top']
  ...
  for i in num: 
    ...
    for arg in arguments:
      len_arg = len(arg)
      arg_match = b[0:len_arg] # this is the part of the string that is the arg
  ...
  mm_read = int(input_arg['flag_mm_read']) # read parameter, if read = 1, then structures from file list  
  qm_read = int(input_arg['flag_qm_read']) # read parameter, if read = 1, then structures from file list  
  mm_read_file = str(input_arg['mm_read_file']) # file with the MM list of structures
  qm_read_file = str(input_arg['qm_read_file']) # file with the qm eneries list 
  ...
  # get the atom mask information for calculating dihedrals etc
  if sys.argv[3] == "-at":
    dih_name = np.genfromtxt(sys.argv[4], dtype='str', delimiter=",", skip_header=1, usecols=0)
    dih_mask = np.genfromtxt(sys.argv[4], dtype='str', delimiter=",", skip_header=1, usecols=1)
    dih_atomtype = np.genfromtxt(sys.argv[4], dtype='str', delimiter=",", skip_header=1, usecols=2)
...
if nbreaks == 1: #only have 1 dataset
  ...
  #make genA input
  genA.make_genA_input(dihnum, mol_name, dih_name, dih_atomtype, dihedral, QMeng, MMeng, nbreaks)
...
if nbreaks > 1:
  ...
  # get MM energy and write trajectory, for nbreaks > 1 can only do mmread =1 or 2 
  MMeng = []; dihedral =[]
  for i in range(nbreaks):
    if mm_read == 1: # get energy from file
      ...
      # get MM energy
      temp = genA.extract_MMenergy(mol_name[i], nconf[i], 1, "temp%s" %(i), 'info')
      MMeng.append(temp)
    elif mm_read == 2: # get energy from file
      temp = genA.extract_MMenergy(mol_name[i], nconf[i], 2, "temp%s" %(i), 'info')
      MMeng.append(temp)
  for i in range(nbreaks):
    if dihnum == 1:
      dih = genA.cal_dih(top[i], "./%s.nc" %(mol_name[i]), dih_mask[i*dihnum:(i+1)*dihnum][0], dihnum)
      dihedral.append(dih)
    else:
      dih = genA.cal_dih(top[i], "./%s.nc" %(mol_name[i]), dih_mask[i*dihnum:(i+1)*dihnum], dihnum)
      dihedral.append(dih)
